...
export function ClientEditPage() {
  ...
  if (clientQuery.isError || !clientQuery.data) {
    return <EmptyState title="Cliente não encontrado" message="Volte para a lista e tente abrir o cadastro novamente." />;
  }
      ...
      <PageHeader
        ...
        action={
          <Link to={`/clients/${clientId}/catalog`}>
            <Button>Mix e preço</Button>
          </Link>
        }
      />
      <Card className="space-y-2">
        <p className="text-sm font-semibold text-[var(--jam-ink)]">Mix e preço do cliente</p>
        <p className="text-sm text-[var(--jam-subtle)]">
          Abra o mix para revisar produtos ativos e preço configurado deste cliente.
        </p>
      </Card>
